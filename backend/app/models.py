...
class User(db.Model):
    ...
    def serialize(self):
        ...
        total_crypto_balance = sum(crypto.amount for crypto in self.cryptos) if self.cryptos else 0.0
...
def handle_deposit(user_id, amount, account_name, account_number):
    try:
        ...
        transaction = Transaction(user_id=user.id, amount=amount, account_name=account_name, 
                                  account_number=account_number, transaction_type='deposit')
...
def handle_withdrawal(user_id, amount, account_name, account_number):
    ...
    if user_balance and user_balance.balance >= amount:
        try:
            ...
            user_balance.balance -= amount
            transaction = Transaction(user_id=user.id, amount=-amount, account_name=account_name,
                                      account_number=account_number, transaction_type='withdrawal')
            db.session.add(transaction)
            db.session.commit()
...
def handle_buy_pdes(user_id, amount, price_per_coin):
    ...
    if user_balance.balance >= total_cost:
        ...
        if not crypto:
            crypto = Crypto(user_id=user.id, crypto_name='Pdes', amount=0.0, account_address='') # No address required yet
            db.session.add(crypto)
        
        crypto.amount += amount
        
        # Record the transaction
        pdes_transaction = PdesTransaction(user_id=user.id, action='buy', amount=amount, 
                                           price=price_per_coin, total=total_cost)
        db.session.add(pdes_transaction)
        db.session.commit()
        
        return {"message": "Purchase successful", "new_balance": user_balance.balance, "new_crypto_amount": crypto.amount}
    else:
...
def handle_sell_pdes(user_id, amount, price_per_coin):
    ...
    if crypto and crypto.amount >= amount:
        ...
        # Record the transaction
        pdes_transaction = PdesTransaction(user_id=user.id, action='sell', amount=amount, 
                                           price=price_per_coin, total=total_sale)
        db.session.add(pdes_transaction)
        db.session.commit()
        
        return {"message": "Sale successful", "new_balance": user_balance.balance, "remaining_crypto_amount": crypto.amount}
    else:
        return {"message": "Insufficient Pdes coins to sell"}

def get_pdes_trade_price():
    # Fetch the latest trade price from CoinPriceHistory
    latest_price = CoinPriceHistory.query.order_by(CoinPriceHistory.timestamp.desc()).first()
    if latest_price:
